%% RUN_uniform_loading_QC: script for example in Section 5.1 (uniaxial loading test), QC solution
...
PotentialI = [1*100,10*100,0.01*1000,0.5]; % for the Inclusion
PotentialM = [1,10,0.01,0.5]; % for the surrounding Matrix
...
%% Interpolation step
FineX = 4*dSizeX; % size of a half of the region along x-axis that will be fully refined
FineY = 4*dSizeY; % size of a half of the region along y-axis that will be fully refined
ExampleType = 'uniform_loading';
switch MeshType
    case 0
        fprintf('Load mesh...\n');
        load('Sa','p','t');
...
for i = 1:length(triangles)
    if length(triangles(i).NeighTriangles)<3
        % Test that given triangle is a boundary one
...
id1 = find(R0QC(1:2:end)<-SizeX+TOL_g & R0QC(2:2:end)<-SizeY+TOL_g); % the left bottom repatom
id2 = find(R0QC(1:2:end)>SizeX-TOL_g & R0QC(2:2:end)<-SizeY+TOL_g); % the right bottom repatom
BCQC = [id1 1 1 0 0
    id2 1 1 u_D 0];

% Find repatom IDs for individual parts of the boundary
IDGamma_1QC = find(abs(R0QC(1:2:end))<SizeX-TOL_g &...
    R0QC(2:2:end)<-SizeY+TOL_g); % Gamma_1\left bottom and right bottom repatoms
IDGamma_2QC = find(R0QC(1:2:end)>SizeX-TOL_g & R0QC(2:2:end)>-SizeY+TOL_g); % Gamma_2 without the right bottom repatom
IDGamma_3QC = find(R0QC(1:2:end)>-SizeX+TOL_g & R0QC(2:2:end)>SizeY-TOL_g); % Gamma_3 without the top left repatom
IDGamma_4QC = find(R0QC(1:2:end)<-SizeX+TOL_g & R0QC(2:2:end)>-SizeY+TOL_g); % Gamma_4 without the left bottom repatom

% Add Gamma_1 constraints: no vertical displacement
BCQC = [BCQC;IDGamma_1QC,zeros(size(IDGamma_1QC)),...
    ones(size(IDGamma_1QC)),zeros(size(IDGamma_1QC)),...
    zeros(size(IDGamma_1QC))];

% Add Gamma_2 constraints: prescribed horizontal displacement
BCQC = [BCQC;IDGamma_2QC,ones(size(IDGamma_2QC)),...
    zeros(size(IDGamma_2QC)),u_D*ones(size(IDGamma_2QC)),...
    zeros(size(IDGamma_2QC))];

% Add Gamma_3 constraints: repatoms on Gamma_3 move vertically as the top left corner repatom does
MasterSlaveQC = [IDGamma_4QC(end)*ones(size(IDGamma_3QC)),IDGamma_3QC]; % master and slave repatom IDs
% Assembly C matrix of linear constraints due to tying conditions
C = zeros(size(MasterSlaveQC,1),2*length(repatoms));
for i = 1:size(MasterSlaveQC,1)
    C(i,2*MasterSlaveQC(i,1)) = 1;
    C(i,2*MasterSlaveQC(i,2)) = -1;
end
C = sparse(C);

% Add Gamma_4 constraints: no horizontal displacement
BCQC = [BCQC;IDGamma_4QC,ones(size(IDGamma_4QC)),...
    zeros(size(IDGamma_4QC)),zeros(size(IDGamma_4QC)),...
    zeros(size(IDGamma_4QC))];
