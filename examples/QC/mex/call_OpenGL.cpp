...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs,
	...
	for (i = 0; i < atomsQC[0]; i++){
		for (j = 0; j < atomsQC[1]; j++){
			atomsQC[(i + NrepAtoms*j) + 2] = (INT)prAtomsQC[i + NrepAtoms*j];
		}
	}
	...
	for (i = 0; i < bonds[0]; i++){
		...
		prPotential = mxGetPr(mxGetFieldByNumber(prhs[3], i, name_Potential));
		for (j = 0; j < 2; j++){
	...
	for (i = 0; i < triangles[0]; i++){
		prVertexAtoms = mxGetPr(mxGetFieldByNumber(prhs[5], i, name_VertexAtoms));
	...
	if ((hsatoms == NULL) || (hatomsQC == NULL) || (hbonds == NULL) ||
		...
		(hZ == NULL) || (hK == NULL) || (hTime == NULL) || (hSize == NULL)){
		delete[] satoms;
		delete[] atomsQC;
		delete[] bonds;
		delete[] bondsQC;
	...
	psatoms = (INT*)MapViewOfFile(hsatoms, FILE_MAP_ALL_ACCESS,
		0, 0, (NsAtoms + 2)*sizeof(INT));
	patomsQC = (INT*)MapViewOfFile(hatomsQC, FILE_MAP_ALL_ACCESS,
		0, 0, (NrepAtoms + 2)*sizeof(INT));
	pbonds = (FLOAT*)MapViewOfFile(hbonds, FILE_MAP_ALL_ACCESS,
		0, 0, (NBonds * 8 + 2)*sizeof(FLOAT));
	pbondsQC = (INT*)MapViewOfFile(hbondsQC, FILE_MAP_ALL_ACCESS,
		0, 0, (NBondsQC + 2)*sizeof(INT));
	ptriangles = (INT*)MapViewOfFile(htriangles, FILE_MAP_ALL_ACCESS,
		0, 0, (Ntriangles * 3 + 2)*sizeof(INT));
	...
	if ((psatoms == NULL) || (patomsQC == NULL) || (pbonds == NULL) ||
		...
		(pZ == NULL) || (pK == NULL) || (pTime == NULL) || (pSize == NULL)){
		CloseHandle(hsatoms);
	...
	CopyMemory((PVOID)ptriangles, triangles, (Ntriangles * 3 + 2)*sizeof(INT));
